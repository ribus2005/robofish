...
int main(int argc, char *argv[])
{
    ...
    double *local_grid = (double*)calloc(ny * nx, sizeof(double));
    double *local_newgrid = (double*)calloc(ny * nx, sizeof(double));
    ...
    #pragma acc data copy(local_grid[0:ny*nx], local_newgrid[0:ny*nx], diff[0:ny*nx])
    {
        double maxdiff;
        ...
        for (;;) {
            ...
            if(niters % 1000 == 0){
                ...
                double* d_grid = (double*)acc_deviceptr(local_grid);
                double* d_newgrid = (double*)acc_deviceptr(local_newgrid);
                double* d_diff = (double*)acc_deviceptr(diff);

                const double alpha = -1;

                cublasDcopy(handle, nx*ny, d_newgrid, 1, d_diff, 1);

                cublasDaxpy(handle, nx * ny, &alpha, d_grid, 1, d_diff, 1);
                
                int maxindex;

                cublasIdamax(handle, nx * ny, d_diff, 1, &maxindex);

                int minindex;

                cublasIdamin(handle, nx * ny, d_diff, 1, &minindex);

                #pragma acc update self(diff[maxindex-1:1])
                #pragma acc update self(diff[minindex-1:1])

                if (fabs(diff[minindex-1]) > fabs(diff[maxindex-1]))maxdiff = fabs(diff[minindex-1]);
                else maxdiff = fabs(diff[maxindex-1]);
